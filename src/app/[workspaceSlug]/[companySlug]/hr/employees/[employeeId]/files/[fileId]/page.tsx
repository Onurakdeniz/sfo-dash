...
export default function EmployeeFileDetailsPage() {
  ...
  const { data: file, isLoading } = useQuery<EmployeeFile | null>({
    queryKey: ["employee-file", workspaceId, companyId, employeeId, fileId],
    queryFn: async () => {
      if (!workspaceId || !companyId) return null;
      const res = await fetch(`/api/workspaces/${workspaceId}/companies/${companyId}/employees/${employeeId}/files/${fileId}`, { credentials: "include" });
      if (!res.ok) return null;
      return res.json();
    },
    enabled: !!workspaceId && !!companyId && !!employeeId && !!fileId,
  });

  const deleteFileMutation = useMutation({
    mutationFn: async () => {
      if (!workspaceId || !companyId) throw new Error("Workspace/company not found");
      const res = await fetch(`/api/workspaces/${workspaceId}/companies/${companyId}/employees/${employeeId}/files/${fileId}`, {
        method: "DELETE",
        credentials: "include",
      });
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["employee-files", workspaceId, companyId, employeeId] });
  ...
  const handleSaveMeta = async () => {
    ...
    try {
      ...
      setIsEditOpen(false);
      queryClient.invalidateQueries({ queryKey: ["employee-file", workspaceId, companyId, employeeId, fileId] });
      queryClient.invalidateQueries({ queryKey: ["employee-files", workspaceId, companyId, employeeId] });
    } catch (e) {
      console.error(e);
  ...
  const headerActions = (
    <div className="flex items-center gap-2">
      <Link href={`/${workspaceSlug}/${companySlug}/hr/employees/${employeeId}/files`}>
  ...
  return (
    <PageWrapper
      ...
      breadcrumbs={[
        { label: contextData?.currentCompany?.name || "Şirket", href: `/${workspaceSlug}/${companySlug}`, isLast: false },
        { label: "Personel", href: `/${workspaceSlug}/${companySlug}/hr/employees/${employeeId}`, isLast: false },
        { label: "Dosyalar", href: `/${workspaceSlug}/${companySlug}/hr/employees/${employeeId}/files`, isLast: false },
