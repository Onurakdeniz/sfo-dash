...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ workspaceId: string; companyId: string; customerId: string; fileId: string }> }
) {
  try {
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session?.user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

    const { workspaceId, companyId, customerId, fileId } = await params;
    ...
    const [row] = await db
      .select()
      .from(customerFile)
      .where(and(eq(customerFile.id, fileId), eq(customerFile.customerId, customerId)))
      .limit(1);
...
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db
      ...
      .where(eq(customerFile.id, fileId))
      .returning();
