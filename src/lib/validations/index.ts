...
export * from './request';
export * from './system';
// Removed talep exports - migrated to request system
...
export function validatePartial<T extends z.ZodObject<any>>(
  ...
  // Ensure the return type matches Partial<z.infer<T>>
  const result = partialSchema.parse(input);
 ...
 */
export function createEnumValidator<T extends z.ZodEnum<any>>(
  schema: T
) {
  return {
    validate: (value: unknown): value is z.infer<T> => {
      return schema.safeParse(value).success;
    },
    parse: (value: unknown, defaultValue: z.infer<T>): z.infer<T> => {
 ...
 */
export function isEnumValue<T extends z.ZodEnum<any>>(
  value: unknown,
  schema: T
): value is z.infer<T> {
  return schema.safeParse(value).success;
