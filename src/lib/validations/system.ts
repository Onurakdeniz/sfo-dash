...
export function isValidEnumValue<T>(schema: z.ZodEnum<[string, ...string[]]>, value: unknown): value is T {
  const result = schema.safeParse(value);
  return result.success;
}

// Helper to get all values from an enum schema
export function getEnumValues<T extends readonly [string, ...string[]]>(schema: z.ZodEnum<T>): T {
  return schema.options;
}

// Helper to get enum value or default
export function getEnumValueOrDefault<T>(
