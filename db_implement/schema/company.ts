 ...
 * Company Schema
 * 
 * This module defines company/organization structure and hierarchy tables.
 * It handles company profiles, departmental organization, and unit management
 * for Turkish business requirements including tax and legal identifiers.
 * 
 * Tables:
 * - companies: Main company/organization profiles with Turkish business data
 * - departments: Organizational departments within companies
 * - units: Sub-units within departments for detailed organizational structure
 */

import { pgTable, serial, varchar, text, timestamp, integer, index, unique, jsonb, pgEnum, check, uuid } from "drizzle-orm/pg-core";
import { sql } from "drizzle-orm";
import { users } from "./user";

// Company status enumeration capturing lifecycle stages
export const companyStatusEnum = pgEnum('company_status', ['active', 'inactive', 'onboarding', 'suspended', 'lead']);
...
export const companies = pgTable('companies', {
  ...
  // Uniqueness constraints for identifiers – kept global because workspace scoping handled elsewhere
...
export const departments = pgTable('departments', { 
  ...
  parentDepartmentId: text('parent_department_id').references(() => departments.id),
